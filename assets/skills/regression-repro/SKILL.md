...
| `references/lab-credentials.md` | shared lab credentials (ARM / BMC) — **local only, gitignored**; copy `lab-credentials.md.example` and fill it from the authoritative sources it names | when a box rejects the credentials a procedure assumes |
