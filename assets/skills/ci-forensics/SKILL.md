...
For "how often / since when / is it ours" use `fsearch` — but read skill
`fsearch-failures` first. It moved to a new path and Python 3.8 in 2026-09, and it has
three traps that each flip the conclusion silently (dead old path that answers every
query with zero rows, ~8-day retention that fakes a "first seen" date, and
`--datefrom`/`--dateto` being accepted but ignored).

> The rule that used to live here — *"a real query takes 70–90 s; an instant return is a
> false negative"* — is **obsolete**. Fsearch2 returns 32 rows in ~6 s. Judge by a
> control query with a term you know exists, never by elapsed time.
