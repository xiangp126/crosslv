...
```bash
~/.claude/skills/noga-lock/scripts/noga_wait.sh -n <machine> -L 8 --hours 8 \
    --then '<provisioning command>'
```

Run it as a `run_in_background` Bash task. Details and the traps: **skill `noga-lock`**.
...
2. Find which candidates are **idle**: `jmake --reg-idle`, and/or
   ...
   confirm with `myssh <cand> hostname` and the newest real session mtime before believing it).
   ...
   - **A same-PSID box may still lack this suite's box-specific `topology_<mode>.xml`** (the
     ...
     `myssh <cand> "ip -br addr; sudo mst status; rdma link; lspci | grep -i mellanox"`).
...
git stash push -u -m "auto-stash before <ticket#> branch"    # only if tracked files are modified

# Branch from the FAILING version, named per THIS repo's own convention
# (they differ — check `git branch` for the local style before inventing one):
#   golan_fw2 style : <ticket#>_<topic>_<name>        e.g. 5184928_emu_eq_peter
#   utopx2    style : fix_<ticket#>_<topic>_<line>    e.g. fix_5184928_emu_eq_master
git checkout -b <ticket#>_<topic>_<you> <tag-or-sha-from-Phase-2>

git submodule update --init --recursive    # submodules' recorded commits may need a fetch
git branch --show-current                  # must print YOUR branch, not empty
...
> harness, launch with `run_in_background: true` and wait for the completion notification.
...
SSH() {
    myssh -o StrictHostKeyChecking=no -o ConnectTimeout=15 \
...
- **Canonical example:** `/auto/fwgwork1/pexiang/bugZilla/5138907_pci_addr_change/tools/`
  (`env_rebuild_102.sh` + `per_run_reset.sh`, l-fwreg-102, BF2 virtio-emu). Shows the
  multi-factor verdict, the before/after PCI snapshot+diff, the pre-fix-binary guard, and the
  hard baseline gate. **Status: structure verified, repro NOT yet confirmed** — the earlier
  "confirmed" claim was the false positive dissected above.
...
> Canonical example: `5138907_pci_addr_change/tools/provision_102.sh`, and the incident
> write-up `5138907_pci_addr_change/REPRO_RECORD_20260727.md` (what each missing component
> looked like as a symptom — worth reading before debugging a "the steps don't work" mystery).
...
- **MFT (`flint`/`mst`/`mlxburn`) disappears after every power-cycle.** Reinstall with
  ...
  Any script that power-cycles must re-ensure MFT before the next device op — see `ensure_mft()`
  in `5138907_pci_addr_change/tools/env_rebuild_102.sh`.
...
Once a fix (in `golan_fw2` / `utopx2`) makes the repro stop firing, commit it in the
project's Gerrit format:
...
```bash
cd <repo>            # /auto/fwgwork1/pexiang/golan_fw2 or utopx2
