...
description: Build firmware from golan_fw, burn it to a local NIC/DPU, and run a utopx test against it, end to end on a dev box. Covers picking the GA baseline and the even/odd version gate, creating jmake-compatible worktrees, the offline image-version gate before burning, the OFED/udriver hand-over order, and the failure signatures for each step. Use when asked to build FW yourself, burn a self-built image, run utopx/traffic_test locally, reproduce on a local card, or when a utopx run dies with "NicPfs list is empty", "Udriver init failed: no devices found", "Found 0 Functions", or an ArmAgent UFATAL.
---
...
`golan_fw_<tag>` / `utopx_<tag>`.

```bash
git -C /path/to/golan_fw2 fetch origin <GA_BRANCH>
git -C /path/to/golan_fw2 worktree add -b <topic> /auto/fwgwork1/$USER/golan_fw_<tag> origin/<GA_BRANCH>
...
`utopx_dump`, the CQE checker's requestor-vs-responder semantics, `stedmp.sh`, packet
reconstruction). See skill **`utopx-traffic-forensics`**, which also works on a log from a machine
you never touched.
...
| `Missing requestor CQE` / `Missing responder CQE` | traffic reached the checker — this is not a setup fault. Which side is reported depends on the opcode, not the direction; see skill `utopx-traffic-forensics` |
...
- Analysing a traffic failure once the run started → skill `utopx-traffic-forensics`.
